...
func ResolveZigProjectImports(
	...
	for _, imp := range imports {
		resolvedPath, ok := resolveImportPath(absPath, imp.Path, ext, suppliedFiles, contentReader)
		if ok {
...
func resolveImportPath(
	...
	resolved, ok := resolveZigImportPath(sourceFile, importPath, fileExt, suppliedFiles, contentReader)
	if !ok {
		return "", false
	}
...
func (r *zigReexportResolver) exportsForFile(filePath string) map[string]string {
	...
	declarations := parseZigConstDeclarations(content)
	exports := r.resolveImportAliases(filePath, declarations.imports)
...
func (r *zigReexportResolver) resolveImportAliasPath(sourceFile string, importPath string) (string, bool) {
	return resolveZigImportPath(sourceFile, importPath, r.fileExt, r.suppliedFiles, r.contentReader)
}

func resolveZigImportPath(
	sourceFile string,
	importPath string,
	fileExt string,
	suppliedFiles map[string]bool,
	...
	if importPath == "std" {
		return resolveStdImportPath(sourceFile, contentReader)
	}

	if isBareZigImportSpecifier(importPath) {
		return resolvePackageImportPath(sourceFile, importPath, fileExt, suppliedFiles, contentReader)
	}

	return "", false
}

func resolveStdImportPath(sourceFile string, contentReader vcs.ContentReader) (string, bool) {
...
func resolvePackageImportPath(
	...
	fileName := importPath + fileExt
	for dir := filepath.Dir(sourceFile); ; dir = filepath.Dir(dir) {
		candidates := []string{
			filepath.Join(dir, fileName),
			filepath.Join(dir, "src", fileName),
			filepath.Join(dir, "lib", fileName),
			filepath.Join(dir, "lib", importPath, fileName),
		}
		for _, candidate := range candidates {
...
func isBareZigImportSpecifier(importPath string) bool {
	return importPath != "" &&
		!strings.ContainsAny(importPath, `/\`) &&
		!strings.HasPrefix(importPath, ".") &&
		filepath.Ext(importPath) == ""
}
...
func parseZigConstDeclarations(sourceCode []byte) zigConstDeclarations {
	...
	walk = func(node *sitter.Node) {
		...
		if node.Type() == "variable_declaration" {
			...
			if ok {
				if importPath, ok := zigImportPathInNode(value, sourceCode); ok {
